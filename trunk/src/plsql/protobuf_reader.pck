...
*/
  ...
  FUNCTION sub_structure_offset(p_buffer IN OUT t_buffer, p_field_number IN PLS_INTEGER, p_index IN PLS_INTEGER) RETURN t_buffer;
...
CREATE OR REPLACE PACKAGE BODY protobuf_reader IS
  ...
  --  c_wire_type_mask RAW(1) := hextoraw('07');
  ...
  */
      ...
      IF utl_raw.bit_and(v_byte, c_end_flag_mask) = c_end_flag_mask THEN
        ...
        IF utl_raw.bit_and(v_byte, c_end_flag_mask) = c_end_flag_mask THEN
          ...
          IF utl_raw.bit_and(v_byte, c_end_flag_mask) = c_end_flag_mask THEN
            ...
            dbms_lob.READ(p_buffer.buffer, v_len, p_buffer.current_offset, v_byte);
            p_buffer.current_offset := p_buffer.current_offset + v_len;
            v_result                := v_result + utl_raw.cast_to_binary_integer(utl_raw.bit_and(v_byte, c_used_bits_mask)) * c_shift_28;
  ...
  PROCEDURE skiprawvarint32(p_buffer IN OUT t_buffer) IS
    v_byte RAW(1);
    v_len  PLS_INTEGER := 1;
  BEGIN
    FOR i IN 1 .. 10 LOOP
  ...
  BEGIN
    CASE p_data_type
      WHEN c_data_varint THEN
        protobuf_reader.skiprawvarint32(p_buffer);
  ...
  FUNCTION sub_structure_offset(p_buffer IN OUT t_buffer, p_field_number IN PLS_INTEGER, p_index IN PLS_INTEGER) RETURN t_buffer IS
      ...
      IF p_field_number = v_field_nr THEN
        IF v_index = p_index THEN
          IF v_data_type != c_data_varint THEN
            raise_application_error(-20000, 'the  value on field index==' || p_field_number || ' is not a Varint');
          END IF;
          RETURN readrawvarint32(p_buffer);
        ELSE
          v_index := v_index + 1;
        END IF;
      END IF;
